...
server.grant(oauth2orize.grant.code((client, redirectUri, user, ares, done) => {
  ...
  db.authorizationCodes.save(code, client.id, redirectUri, user._id, user.username, (error) => {
    if (error) return done(error);
...
server.grant(oauth2orize.grant.token((client, user, ares, done) => {
  ...
  db.accessTokens.save(token, user._id, client.clientId, (error) => {
    if (error) return done(error);
...
module.exports.authorization = [
  ...
  server.authorization((clientId, redirectUri, done) => {
    ...
    console.log ('AUTHORIZARTION')
    console.log (user)
    // Auto-approve
    if (client.isTrusted) return done(null, true);
    
    db.accessTokens.findByUserIdAndClientId(user._id, client.clientId, (error, token) => {
      // Auto-approve
